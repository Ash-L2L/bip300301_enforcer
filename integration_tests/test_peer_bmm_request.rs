...
use bip300301_enforcer_lib::{
    ...
    proto::{
        ...
        mainchain::{
            ...
            GetChainTipRequest, SendTransactionRequest,
        },
    },
};
...
use crate::{
    ...
    mine,
    setup::{DummySidechain, EnforcerOpts, Mode, Network, Sidechain},
...
pub const TEST_NAME: &str = "peer_bmm_request (GBT)";
...
impl PreSetup {
    ...
    async fn setup(
        ...
        let sender = {
            let enforcer_opts = EnforcerOpts {
                p2p_broadcast_peer_ports: vec![self.miner.reserved_ports.bitcoind_listen.port()],
            };
            self.sender
                .setup(Mode::GetBlockTemplate, enforcer_opts, res_tx.clone())
                .await?
        };
        let miner = self
            .miner
            .setup(Mode::GetBlockTemplate, EnforcerOpts::default(), res_tx)
            .await?;
    ...
    if post_setup
        ...
        .send_transaction(SendTransactionRequest {
            destinations: HashMap::from([(
                post_setup.sender.receive_address.to_string(),
                123_456_u64,
            )]),
            ..Default::default()
        })
